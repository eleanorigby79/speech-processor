...
#define REC_FREQ                48000     // Audio recording frequency in Hz
#define INTERNAL_BUFF_SIZE      192        // PDM buffer input size
#define PCM_OUT_SIZE            48     // PCM buffer output size
...
{
  WaveRecorderInit(REC_FREQ*2,16, 1);
...
static void WaveRecorder_SPI_Init(uint32_t Freq)
{
  ...
  /* SPI configuration */
  SPI_I2S_DeInit(SPI2);
  I2S_InitStructure.I2S_AudioFreq = REC_FREQ;
